...
- Strategy Staking Module: a smart contract that allows players to sponsor specific strategies (so that they are added to the vault) by staking their ELX, making money if they do well and losing money if they don't.
...
Players deploying "branded" vaults will use a separate registry to allow permissioned endorsement of vaults for their product
...
### Deposits / Mints
Users can deposit ASSET tokens to receive elxnTokens (SHARES).
...
Strategies are completely independent smart contracts that can be implemented following the [Tokenized Strategy](https://github.com/elision-labs/tokenized-strategy) template or in any other way.
